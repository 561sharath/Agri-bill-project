...
const RecordPaymentModal = ({ onClose, onSave, initialFarmer = null }) => {
    ...
    const handleFullPayment = useCallback(() => {
        if (selectedFarmer?.creditBalance) {
            setValue('amount', selectedFarmer.creditBalance);
        }
    ...
    const isOverpaying = selectedFarmer && Number(watchAmount) > selectedFarmer.creditBalance;
...
const Payments = () => {
    ...
    const fetchPayments = useCallback(async (p = page, f = filters) => {
        setLoading(true);
        try {
    ...
    const methodBadge = useCallback((method) => {
        const map = {
            ...
            <div className="p-4 sm:p-6 flex flex-col gap-6 animate-fade-in max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {[
                        ...
                        { label: 'Showing Now', value: `${payments.length} records`, icon: 'list', color: 'text-blue-600', bg: 'bg-blue-50 dark:bg-blue-900/10' },
                    ].map((s, i) => (
                                ...
                                {loading ? (
                                    ...
                                    payments.map(payment => (
                                        <tr key={payment._id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                                            ...
                                            <td className="px-5 py-4 text-xs text-slate-500 italic max-w-[150px] truncate" title={payment.notes}>{payment.notes || '—'}</td>
